...
@ManagedBean
@SessionScoped
public class ActiviteConsolidationController {
    ...
    public ActiviteConsolidationController() {
    }
    ...
    private void init() {
        ...
        try {
            ...
            if (!axes.isEmpty()) {
                ...
                List<Probleme> pbs = problemeFacadeLocal.findByAxeDistrict(axe, SessionMBean.getDistrict(), 1);
                if (pbs.isEmpty()) {
                    pourcentageAxe = 0d;
                } else {
                    Integer conteur = 0;
                ...
                if (!sousaxes.isEmpty()) {
                    ...
                    activites = activiteFacadeLocal.find(SessionMBean.getDistrict(), sousaxe, 1);

                    List<Probleme> pbs1 = problemeFacadeLocal.findBySousAxeDistrict(sousaxe, SessionMBean.getDistrict(), 1);
                    if (!pbs1.isEmpty()) {
    ...
    public void updateProbleme() {
        try {
            ...
            if (indicateur.getIdindicateur() != null) {
                ...
                if (sousaxe != null) {
                    problemes = problemeFacadeLocal.find(indicateur, district, 1);

                    if (!problemes.isEmpty()) {
                        probleme = problemes.get(0);
                    } else {
                        //JsfUtil.addErrorMessage("L'indicateur est non evalué pour le district");
                    }
                }
            }
        } catch (Exception e) {
    ...
    public void updateSelect() {
        //showSelectActivite = showSelector;
    }
        ...
        try {
            ...
            if (!indicateurs.isEmpty()) {
                for (Indicateur i : indicateurs) {
                    List<IndicateurDistrict> ids = indicateurDistrictFacadeLocal.findByDistrictIndicateurObservation(district, i, 1);
                    if (!ids.isEmpty()) {
        ...
        try {
            if (!sousaxes.isEmpty()) {
                ...
                if (sousaxe != null) {
                    ...
                    List<Probleme> pbs = problemeFacadeLocal.findBySousAxeDistrict(sousaxe, SessionMBean.getDistrict(), 1);
            ...
            if ("Edit".equals(mode)) {
                ...
                if (isCoasted) {
                    if (!activiteElementCouts.isEmpty()) {
                        for (ActiviteElementCout a : activiteElementCouts) {
                            if (a.getIdactiviteElementCout() != null) {
                                activiteElementCoutFacadeLocal.edit(a);
                            } else {
                                a.setIdactiviteElementCout(activiteElementCoutFacadeLocal.nextId());
                                activiteElementCoutFacadeLocal.create(a);
                            }
                        }
                    }
                }
        ...
        try {
            if (activite != null) {
                ...
                if (chronogrammeFacadeLocal.findByActivite(activite).isEmpty()) {
                    ...
                    List<ActiviteElementCout> activiteElementCouts = activiteElementCoutFacadeLocal.findByActivite(activite);
                    if (activiteElementCouts.isEmpty()) {
                        for (ActiviteElementCout a : activiteElementCouts) {
                            activiteElementCoutFacadeLocal.remove(a);
                        }
                    }
    ...
    public ActiviteElementCout getActiviteElementCout() {
        return activiteElementCout;
    }

    public void setActiviteElementCout(ActiviteElementCout activiteElementCout) {
        this.activiteElementCout = activiteElementCout;
    }
